...
namespace AutoSinkerTester
{
    ...
    public class UnitTest1
    {
        ...
        public void PrivateTest1()
        {
            ...
            int[] answers = new int[] { 2, 2, 5, -1, -1, -1, 0 };
            for (int i = 0; i < cities.Length; i++)
            {
                graph.Add(cities[i], new Vertex());
                graph[cities[i]].price = prices[i];
            ...
            trips[0] = new Tuple<string, string>(cities[2], cities[3]);
            trips[1] = new Tuple<string, string>(cities[2], cities[3]);
            trips[2] = new Tuple<string, string>(cities[2], cities[5]);
            trips[3] = new Tuple<string, string>(cities[7], cities[1]);
            trips[4] = new Tuple<string, string>(cities[7], cities[3]);
            trips[5] = new Tuple<string, string>(cities[7], cities[6]);
            trips[6] = new Tuple<string, string>(cities[7], cities[7]);
            AutoSinker AS = new AutoSinker();
            int[] result = AS.CalculateTrips(graph, trips);
